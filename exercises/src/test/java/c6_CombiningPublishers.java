...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        Flux<Void> tasks = taskExecutor().flatMap(value -> value);
    ...
    public void streaming_service() {
        ...
        Flux<Message> messageFlux = streamingService().flatMapMany(val -> val);
     ...
     * FlatMap runs things in paralell (order is not guarenteed), concatMap runs things one-by-one (order is guarenteed)
     * - What is difference between concatMap() and flatMapSequential()?
     * FlatMapSequential still runs things in parallel, but the order is preserved.
     * - Why doesn't Mono have concatMap() operator?
     * It only contains one or zero objects, thus there is nothing to concatenate
     */
    @Test
    public void task_executor_again() {
        Flux<Void> tasks = taskExecutor().concatMap(event -> event);
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal , message)-> {
    ...
    public void acid_durability() {
        //todo: feel free to change code as you need
        Flux<String> committedTasksIds = tasksToExecute().concatMap(task -> task).concatMap(task ->

                    commitTask(task).thenReturn(task)
                );
    ...
    public void cleanup() {
        ...
        Flux<String> stream =
                Flux.using(
                        ...
                        monoFlux -> monoFlux.flatMapMany(Function.identity()),
                        ignored -> StreamingConnection.closeConnection()
                );
